...
resource "null_resource" "download_k3s_yaml" {
  provisioner "local-exec" {
    command = "sshpass -p '${var.password}' scp -i ${path.module}/sshkey/key -o StrictHostKeyChecking=no ${var.user}@${tencentcloud_instance.web[0].public_ip}:/tmp/k3s.yaml ${path.module}/config.yaml"
  }
...
#   provisioner "local-exec" {
#     command = "export KUBECONFIG=${path.module}/config.yaml"
#   }
#   depends_on = [null_resource.download_k3s_yaml]
# }
